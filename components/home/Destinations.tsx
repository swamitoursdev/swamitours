  ...
  return (
    <section id="destinations" className="w-full px-5 sm:px-8 py-16 sm:py-20">
                ...
                {showImage && (
                  <>
                    <Image
                      ...
                      className="kenburns-mobile object-cover transition-transform duration-500 ease-out group-hover:scale-105"
                      style={{ animationDelay: `${(i % 4) * 1.2}s` }}
                      onError={() =>
                        setBroken((prev) => {
                          const next = [...prev];
                          next[i] = true;
      ...
      <style jsx global>{`
        ...
        /* Cards have no hover on touch devices, so give the image its own
           slow breathing zoom below the sm breakpoint instead. Above that,
           the existing group-hover:scale-105 handles it. */
