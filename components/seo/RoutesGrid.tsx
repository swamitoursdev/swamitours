...
export default function RoutesGrid({
  ...
  // If the user is on a city-scoped grid and searches that same city
  // (e.g. typing "Mumbai" on a Mumbai page), every chip already matches —
  // treat it as "show everything" instead of a no-op filter that looks broken.
  const isSelfCityQuery =
    !!fromCity && rawQuery.length > 0 && rawQuery === fromCity.toLowerCase();

  const query = isSelfCityQuery ? "" : rawQuery;
  ...
  const renderChip = (r: { from: string; to: string }, i: number) => {
    const slug = routeSlug(r.from, r.to);
    const isPressed = pressedKey === slug;
    return (
      <Link
        key={slug}
        href={`/${slug}`}
        onClick={handleChipClick(slug, `/${slug}`)}
        style={{
      ...
      <div className="mx-auto w-full max-w-[1800px] px-4 sm:px-8 lg:px-14 xl:px-20">
        ...
        {filtered.length > 0 ? (
          <>
            ...
            >
              {primary.map((r, i) => renderChip(r, i))}
            </div>
            ...
            {shouldCollapse && (
              <div className="mt-6 flex justify-center">
                ...
                >
                  {expanded ? "Show less" : `Show all ${filtered.length} routes`}
                  <svg
